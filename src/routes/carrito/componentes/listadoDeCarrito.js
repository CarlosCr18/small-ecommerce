...
const ListadoDeCarrito = ({ arrayDeCarrito, useDispatch, addItem, deleteItem }) => {
  const dispatch = useDispatch();
  const [cantidades, setCantidades] = React.useState(arrayDeCarrito.map((el) => el.qty));
          ...
          <div key={element.product_id}>
            ...
            <div className={styles.controles}>
              ...
              <button
                className="eliminar"
                onClick={() => {
                  setCantidades(cantidades.filter((cantidad, i) => i !== index));
